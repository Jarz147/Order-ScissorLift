...
export default function Booking() {
  ...
  const checkConflict = async (start, end) => {
    ...
    if (data.length > 0) {
      setConflict(true)
      setError('Scissor lift sudah dipesan pada rentang tanggal tersebut (atau sedang menunggu approval).')
      return false
    }
    setConflict(false)
    setError('')
  ...
  const handleSubmit = async (e) => {
    ...
    if (insErr) {
      if (insErr.code === '23P01') {
        setConflict(true)
        setError('Scissor lift sudah dipesan pada rentang tanggal tersebut (data terbaru). Pilih tanggal lain.')
      } else {
